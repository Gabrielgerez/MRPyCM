...
def test_V_R_norm():
    assert np.isclose(Solver.V_R.norm(), 5.636483265893928, rtol=epsilon, atol=epsilon)

def test_V_R_integrate():
    assert np.isclose(Solver.V_R.integrate(), -446.7612555592769, rtol=epsilon, atol=epsilon)
    
def test_zeroth_energy():
    assert np.isclose(Solver.energy_convergence[0], -0.3606291269670691, rtol=epsilon, atol=epsilon)
    
def test_first_iteration():
    assert np.isclose(Solver.energy_convergence[-1], -0.24300207105286137, rtol=epsilon, atol=epsilon)
