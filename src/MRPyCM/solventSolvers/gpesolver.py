...
class GPESolver:
    ...
    def __init__(
        self, rho, eps, Poisson_operator, Derivative_operator, max_iter=100, hist=0
    ):
        self.Density = rho
        ...
        self.rho_eff = (4 * np.pi) * (
            (self.Density * (self.Permittivity) ** (-1)) - (self.Density)
        )
        self.iterations = 0
    ...
    def computeGamma(self, V_tot, epsilon):
        gamma = vp.dot(
            vp.gradient(self.D, V_tot), vp.gradient(self.D, self.Permittivity)
        ) * (self.Permittivity ** (-1))
    ...
    def iterateEquation(self, prec):
        print(
            f"Iter.{' '*2}Norm{' '*12}Update{' '*10}Energy (a.u.){' '*3}Energy update (a.u.)"
        )
        print(f"{'-'*75}")
        ...
        dE_r = E_r
        print(
            f"{0:2d}{' '*5}{self.V_R.norm():14.7e}  {update:14.7e}  {E_r:14.7e}  {dE_r:14.7e}"
        )
        for i in range(self.max_iter):
            ...
            self.energy_convergence.append(E_r)
            print(
                f"{i+1:2d}{' '*5}{self.V_R.norm():14.7e}  {update:14.7e}  {E_r:14.7e}  {dE_r:14.7e}"
            )
